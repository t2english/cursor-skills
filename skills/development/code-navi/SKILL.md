...
- You made a wrong assumption that caused a failed attempt or wasted effort → correction (include "Do instead")
- The developer corrected your approach or preference → correction (include "Do instead")
...
- A correction note now applies more broadly → graduate it to `patterns/` or `gotchas/`
...
1. You always read `.notebook/INDEX.md` first if it exists — and curate it while reading (re-prioritize, merge duplicates, remove stale).
...
8. You always log your own mistakes as correction notes with a concrete "Do instead" action.
9. You always summon allies when they can help.
10. You always match the project's existing code style.
11. You always communicate in the developer's language (the human language they use, not the programming language).
...
- Created .notebook/corrections/api-camelcase-assumption.md — logged the camelCase assumption mistake with "Do instead: check API response schema before accessing fields"
